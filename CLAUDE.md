...
- **Kiosk system on-screen keyboard — ABANDONED; calendar connect is app-only.** Third-party OAuth pages (Google/MS login in kiosk Chromium) were the only wall text-entry our in-app React OSK (`src/components/setup/onscreen-keyboard.tsx`) can't cover. Explored `onboard` (X11-wide OSK): PR #109's `.xinitrc` launch wiring (`exec dbus-run-session -- "$0"`) **black-screened the kiosk on hardware** (reverted, PR #111). A live SSH session then found onboard CAN work (needs `libglib2.0-bin` + `glib-compile-schemas` + a session bus + `force-to-top`) but **auto-show over Chromium never worked reliably**. Decision: drop the system OSK entirely — **all calendar connect (Google/Microsoft/CalDAV) moves to the mobile app** (better OAuth UX: phone keyboards, password managers, passkeys; no creds typed on a shared wall). Wall Settings shows status + sync/disconnect but no connect action. `onboard` packages removed from the host payload. LESSON stands: never put unverified X11 changes in a base image (`.xinitrc` isn't OTA-updatable; a bad one bricks a networkless device — always ship images key-only-SSH so there's a recovery channel).
